...
## アーキテクチャ（レイヤ規約・新機能はこの層に置く）

ヘキサゴナル（ports & adapters）。**依存は内側（core）へ一方向**。新しい実装を書くときは
まず「どの層か」を決める。

```
src/protocol/  Bambu ワイヤモデル（topics, messages）。leaf、依存なし
src/core/      純粋ドメイン：types / ports（PrinterPort, Notifier, Clock, QueueStore）/
               services（dispatcher, eta, ams）。I/O 禁止・外部ライブラリ禁止
src/db/        永続化アダプタ。Repo implements core QueueStore
src/orchestrator/ I/O アダプタ（MQTT/FTPS クライアント, MqttFtpsPrinter が PrinterPort 実装）
src/stub/      仮想プリンター（dev/test double）。protocol に依存
src/verify/    シナリオランナー + Sut + StubSut（合成/composition root）
```

**規約**：
- ドメインのルール（ディスパッチ・ETA・AMS一致・runout tier・color policy など）は **core/**
  に置き、散逸させない。core は db/stub/mqtt を import しない（ports 経由）。
- 外部 I/O（MQTT/FTPS/HTTP/webhook/SQLite）は必ず port の adapter として実装する。
- 通知は `Notifier` port 経由（ad-hoc に配線しない）。時刻依存は `Clock` port。
- 新しい永続クエリは Repo に足し、dispatcher が使うなら `QueueStore` にも追加する。
