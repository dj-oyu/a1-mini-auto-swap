...
- **フィラメント確認・変更UI**（スロットごとの色スウォッチ＋AMS実装填からの選択ドロップダウン）。✅実装済み
- プロジェクト一覧（policy設定、所属プレートの進捗・全体ETA）。✅実装済み（`GET /projects`：作成フォーム＋policyトグル＋プレート進捗。全体ETAは静的見積り集計）
- 各カードに 中止／リトライ／削除。✅リトライ・削除は実装済み（中止は printer-stop API 待ち）
