...
- 実測: 初回フレームまで約3.1秒、1080p で約246KB、約1fps。**Bun の TLS 受信方向は問題なし**
  （FTPS の LIST と同様）。
- **実装（リレー方式）**: ワイヤコーデック（80B認証パケット + フレームパーサ）は純粋関数として
  `src/orchestrator/camera.ts`。接続ライフサイクルは `src/orchestrator/camera-relay.ts` の
  `CameraRelay` — **上流接続は常に最大1本**を保持し、全消費者（`GET /api/printer/camera.mjpeg`
  の MJPEG ストリームを開いた全ブラウザタブ + 単発スナップショット）へフレームを再配布する。
  最初の購読で接続し、最後の解除から linger（既定15s）後に切断（モーダル再開やストリーム直後の
  スナップショットが同じスロットを再利用でき、希少なカメラスロットのチャーンを防ぐ）。上流切断時は
  購読者が居ればバックオフ再接続（1s→2s→…上限15s）、居なければ停止。カメラスロットを1本に抑える
  ことで Bambu Studio のライブビューとの競合も最小化。
- スナップショット（`GET /api/printer/snapshot`、webhook添付/verify用）も同じリレーの
  `snapshot()` 経由 — 最新フレームがあれば即返し、無ければ一時購読で1枚取得。失敗は常に null/404
  にデグレードし呼び出し元を壊さない（spec 20.8）。アクセスコードはログに出さない。
