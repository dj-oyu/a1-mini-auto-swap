...
プレート交換シーケンス自体は本仕様の対象外とし、既存の `SWAP_GCODE_SEQUENCE` を独立ジョブとして同じ配信方式(`gcode_line` / `gcode_file`)で実行する。軸テスト(振動耐性)とプレート交換テスト(機構動作)は目的が異なるため、レイヤー分割は軸テストのみを対象とし、プレート交換テストは1本の独立ジョブとして扱う。
